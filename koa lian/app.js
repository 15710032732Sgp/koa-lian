...
router.post('/api/add',async ctx => {
    ...
    if(username && password && idcard){ 
        ...
        if(user.data.length){
            ...
            let data = await query('insert into userlist (username,password,address,idcard,phone,sex,create_time) values (?,?,?,?,?,?,?)',[username,password,address,idcard,phone,sex,create_time]);
            if(data.msg === 'error'){
                ctx.body = {
                    code:0,
                    msg:error
